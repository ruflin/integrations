...
func stripReferencesToEventModuleInQuery(object mapStr, objectKey, moduleName string, datasetNames []string) (mapStr, error) {
	...
	query = strings.ReplaceAll(query, " :", ":")
	query = strings.ReplaceAll(query, `"`, "")
	if strings.Contains(query, "event.module:"+moduleName) && (strings.Contains(query, "metricset.dataset:") || strings.Contains(query, "fileset.dataset:")) {
		query = strings.ReplaceAll(query, "event.module:"+moduleName, "")
		query = strings.ReplaceAll(query, "metricset.dataset:", fmt.Sprintf("datastream.dataset:%s.", moduleName))
		query = strings.ReplaceAll(query, "fileset.dataset:", fmt.Sprintf("datastream.dataset:%s.", moduleName))
